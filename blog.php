        ...
        <div class="col-xl-7 text-center">
          <div class="section__heading">
            ...
            <p class="mt-20" style="color:#7a8aab; font-size:18px;">
              Deep-dives into Generative AI, cutting-edge cybersecurity protocols, and next-generation DevOps workflows written by our core engineering team.
            </p>
          </div>
        </div>
      </div>
      <div class="row mt-none-30">
        <?php if ($searchQuery !== ''): ?>
          <div class="col-xl-12 mt-30">
            <p style="font-size:16px; color:#7a8aab;">Showing results for: <strong><?= htmlspecialchars($searchQuery, ENT_QUOTES) ?></strong></p>
          </div>
        <?php endif; ?>
            ...
            <div class="col-xl-4 col-lg-6 mt-30">
              <article class="premium-case-card" style="transition:0.3s;">
                <div class="case-card__img">
                  <a href="<?= $postUrl ?>">
                    <img loading="lazy" alt="<?= htmlspecialchars($post['title'], ENT_QUOTES) ?>" src="<?= $base_url ?>/assets/images/blog/<?= htmlspecialchars($post['image'], ENT_QUOTES) ?>" style="width:100%; height:250px; object-fit:cover;">
                  </a>
                </div>
                <div class="premium-case-card__body">
                  <span class="premium-case-tag" style="background: rgba(8, 106, 216, 0.2); color: #38bdf8;"><?= htmlspecialchars($post['category'], ENT_QUOTES) ?></span>
                  <h3><?= htmlspecialchars($post['title'], ENT_QUOTES) ?></h3>
                  <p><?= htmlspecialchars($post['excerpt'], ENT_QUOTES) ?></p>
                  <div class="premium-stat-row">
                    <div class="premium-stat">
                      <div class="number" style="font-size:18px;"><?= htmlspecialchars($post['author'], ENT_QUOTES) ?></div>
                      <div class="label">Author</div>
                    </div>
                    <div class="premium-stat">
                      <div class="number" style="font-size:18px;"><?= htmlspecialchars(date('M j, Y', strtotime($post['date'])), ENT_QUOTES) ?></div>
          ...
          <div class="col-12 text-center">
              <div class="pagination-wrapper">
                  <?php if ($currentPage > 1): ?>
                      <a href="?page=<?= $currentPage - 1 ?><?= $searchQuery ? '&q='.urlencode($searchQuery) : '' ?>" class="site-btn transparent" style="padding: 10px 20px;">Prev</a>
                  <?php endif; ?>
                  
                  <span style="font-size:18px; font-weight:bold; margin:0 20px;">Page <?= $currentPage ?> of <?= $totalPages ?></span>
                  
                  <?php if ($currentPage < $totalPages): ?>
                      <a href="?page=<?= $currentPage + 1 ?><?= $searchQuery ? '&q='.urlencode($searchQuery) : '' ?>" class="site-btn transparent" style="padding: 10px 20px;">Next</a>
                  <?php endif; ?>
              </div>
          </div>
      </div>
